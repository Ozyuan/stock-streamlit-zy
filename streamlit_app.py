...
{{"summary": "两三句话的整体摘要，说清楚今天大致发生了什么", "reason": "结合成交量和新闻分析可能的原因；如果新闻和价格变化关联不明显，要明确说'现有信息不足以确认具体原因，仅供参考'", "watch": ["值得关注的点1（简短一句话）", "值得关注的点2"]}}"""
    try:
        ...
        # Gemini 出错时会在 data["error"] 里给出具体原因，先检查这个
        ...
        if not candidates:
            # 常见于内容被安全过滤器拦截，或返回结构异常
            reason = data.get("promptFeedback", {}).get("blockReason", "未知（返回内容里没有candidates字段）")
            return {"error": f"没有返回结果，原因：{reason}"}

        parts = candidates[0].get("content", {}).get("parts", [])
        if not parts:
            finish_reason = candidates[0].get("finishReason", "未知")
            return {"error": f"返回内容为空，finishReason：{finish_reason}"}
...
# ---- 5. K线（辅助，默认收起，勾选后才真正拉取数据，节省API额度）----
st.markdown("#### 📉 K线图（辅助参考）")
