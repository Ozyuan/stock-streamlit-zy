...
def nearest_bar_index(news_time_str: str, bar_times: pd.Series):
    """把新闻时间就近匹配到某根K线，供图上打点用；存在一定时区误差，仅作大致参考。"""
    try:
...
with left:
    ...
    # 新闻打点：就近匹配到某根K线，鼠标悬停显示标题（这是跟专业软件不同的地方——把"为什么"直接标在图上）
    news_items_for_chart = get_news(symbol)
    mk_x, mk_y, mk_text = [], [], []
    for n in news_items_for_chart:
    ...
    fig.update_layout(
        template=template, height=560, margin=dict(l=10, r=50, t=40, b=10),
    ...
    st.caption("提示：滚轮/在图表中间拖动=整体缩放平移；把鼠标放在最右边价格数字那一栏上下拖=单独缩放价格轴。右上角工具栏有画线工具，双击图表可还原。🟡菱形=新闻，悬停看标题。")
